...
const AnimeListItems = ({ title, page, animes, setPage, maxPages, genre = false, schedule = false, myList = false }: {
    ...
    const { titleLanguageJP } = useTitleLanguageToggle();
    ...
    return (
        <div className="cursor-pointer select-none w-screen ">
            <div className={`py-4 ${schedule ? " " : "px-15"} w-full flex justify-center gap-15 items-center mx-auto`} >
                ...
                <span className={` w-full font-bold text-center ${schedule ? "text-2xl my-6 bg-primary-dark py-2 " : "text-4xl py-5"}`}>
                    {title}
                </span>
            </div>
            <div className="flex flex-wrap justify-center md:gap-6 gap-4 max-w-full  mx-auto">

                {animes?.length === 0 && <AnimeListLoader count={12} />}

                {animes?.map((anime, index) => (
                    <div key={index}>
                        <div
                            className="hover:-translate-y-2 transition-transform duration-200 rounded-2xl border-3 border-primary-dark shadow-md dark:border-primary-dark hover:scale-[1.02] h-54 w-38 hover:ring-3 hover:ring-primary-active md:w-70 md:h-105 max-md:overflow-hidden relative
                      "
                        ...
                        >
                            <Image
                                ...
                                priority={index < 6}
                                fetchPriority="high"
                                className="h-full md:h-84 w-full  md:p-4  object-cover bg-linear-to-br from-black via-primary-dark/35  to-black rounded-t-2xl transform-[translateZ(0)] will-change-transform aspect-3/4 "
                            />
                            <div className="md:text-[18px] text-xs md:px-1 text-center  md:border-t-4 md:py-3 py-2 border-primary-dark  w-full md:h-18 overflow-hidden  h-12 max-md:relative max-md:bottom-12 max-md:bg-black/80 max-md:z-10 line-clamp-2"><span className="w-full text-text-muted line-clamp-2 font-bold ">{titleLanguageJP ? anime.title : anime.title_english ? anime.title_english : anime.title}</span></div>
                        </div>
                        <div className="flex justify-center items-center gap-1.5 md:text-lg text-xs  w-full ">
                            <div className="flex items-center gap-1">
                                ...
                                <span className="text-text-muted py-1"> {anime.score ?? "-"} </span>
                            </div>
            ...
            {(setPage && page && maxPages) &&
                <div className="w-full flex justify-center items-center pb-20 md:pb-12  my-4 gap-4 ">
                    ...
                    <button
                        onClick={() => { if (page > 1 && page <= maxPages) setPage(page - 1); else setPage(1);}}
                    ...
                    <button
                        onClick={() => { if (page >= 1 && page < maxPages) setPage(page + 1); else setPage(maxPages); }}
